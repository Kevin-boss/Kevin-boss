...
| YouTube, TikTok, Facebook Pages, Instagram, LinkedIn, and X publishing | Official API, OAuth, media-upload, and webhook capabilities have been researched and captured in `docs/INTEGRATION_API_RESEARCH.md`. The product now contains connection readiness, provider-contract, publishing-plan, approval, scheduling, callback, idempotency, retry, and audit scaffolding. Credentials are requested and stored server-side only when the user elects to activate the selected platform(s); configured redirect URLs, required platform review, and final provider-execution adapters remain activation requirements. |
