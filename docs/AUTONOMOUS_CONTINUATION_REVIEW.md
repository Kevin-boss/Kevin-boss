...
| Social OAuth account management | Official provider contracts, PKCE URL construction helpers, workspace state schema, single-use server-only callback-state consumption, readiness UI, connected-account assignment, local disconnect, approval gates, idempotent attempts, retry, and schedule lifecycle coverage are in place. | A selected platform requires its official application credentials, platform approval where applicable, and a verified server-side token-exchange/account-discovery adapter. | Authorization redirects are fail-closed; no provider code, verifier, token, or account is exposed or fabricated. |
| Social provider execution | Content calendar, variants, review, matching connected-account selection, dispatch readiness, queueing, local rescheduling, guarded cancellation, and failed-attempt retry controls are covered without outbound calls. | Per-platform token handling, upload/post adapters, public callback deployment, and validated official credentials. | Rescheduling retains approval and provider-readiness checks. Cancellation removes only the persisted task UID for an unsent plan, invalidates local queued attempts, and preserves published and cancelled history. A retry queues a local attempt rather than representing provider publication success. |
