...
| Social provider execution | Content calendar, variants, review, matching connected-account selection, dispatch readiness, queueing, scheduling, rescheduling, guarded cancellation, and failure retry are covered without outbound calls. | Per-platform token handling, upload/post adapters, public callback deployment, and validated official credentials. | Cancellation removes only the persisted task UID for an unsent plan, invalidates local queued attempts, and preserves published and cancelled history. Queue records do not represent publication success; posts are never marked published without an official provider response. |
