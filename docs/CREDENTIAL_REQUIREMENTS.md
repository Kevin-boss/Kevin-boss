...
The currently configured credential-free paths use the built-in application services and the browser-local WebM renderer. Official social publishing has documented provider contracts, a connection-readiness surface, account/approval/attempt records, and a durable dispatch callback foundation. Provider credentials alone do **not** enable an authorization redirect: each platform remains fail-closed until its verified server-side token-exchange and account-discovery adapter is implemented and validated. PKCE URL construction and callback-state consumption remain internal server-side scaffolding only; a callback state is accepted only for its owning workspace, before expiry, and once, with a guarded completion update to reject concurrent reuse. No secret, code verifier, access token, or refresh token is returned to the browser. External post creation remains safely disabled until the user selects a platform for publishing, provides that platform's official application credentials, completes the required OAuth and platform-review path through an enabled adapter, and connects an approved account. The official public Hugging Face endpoint remains the default public-provider reference; the included Kokoro worker remains an optional self-hosted path for work that needs the private consent-controlled contract.
...
| Official social OAuth initiation and callback-state consumption | Scaffolded and fail-closed | Provider authorization does not start until a verified server-side token-exchange and account-discovery adapter is enabled for the selected platform; credentials alone cannot create an external redirect. Callback state is validated and consumed exactly once inside server-side adapter code, with the PKCE verifier never exposed to the browser. |
