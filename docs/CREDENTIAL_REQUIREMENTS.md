...
| Official social OAuth initiation | Scaffolded and fail-closed | Provider authorization does not start until a verified server-side token-exchange and account-discovery adapter is enabled for the selected platform; credentials alone cannot create an external redirect. |
