...
export default function VoiceCaptions() {
  ...
  const voiceQualityLabel = !selectedVoice ? "Choose an approved voice" : usesPublicModelDefaultVoice ? "Public model-default voice" : "Private neural-voice route";
            ...
            <CardContent className="p-5">
              ...
              <form className="mt-4 space-y-3" onSubmit={submitSynthesis}>
                <div className="rounded-lg border border-white/8 bg-slate-950/35 p-3 text-xs text-slate-400" aria-live="polite">
                  {selectedVoice ? <><span className="font-medium text-slate-200">{selectedVoice.name}</span><span className="block pt-1">{selectedVoice.providerId ? `Ready provider #${selectedVoice.providerId} · ${voiceQualityLabel}` : "No enabled commercial-use provider matches this voice yet."}</span><span className={`mt-2 inline-block rounded px-1.5 py-1 text-[10px] ${usesPublicModelDefaultVoice ? "bg-cyan-300/10 text-cyan-100" : "bg-violet-300/10 text-violet-100"}`}>{usesPublicModelDefaultVoice ? "Consistent public fallback" : "Natural-delivery controls require a private worker"}</span></> : "Choose an approved voice from the catalog to enable synthesis."}
                </div>
                ...
                {usesPublicModelDefaultVoice && <p className="rounded-lg bg-cyan-300/7 p-2 text-[11px] leading-4 text-cyan-100">This public-provider route uses the registered model-default voice. It intentionally ignores private-worker delivery controls; choose an approved private neural voice only after its consent-controlled worker is deployed.</p>}
        ...
        <main className="space-y-5">
          <Card className="border-white/7 bg-white/[.025] shadow-none">
            <CardContent className="p-0">
              ...
              {voices.isLoading ? <div className="grid min-h-52 place-items-center text-center" role="status" aria-live="polite"><div><Loader2 className="mx-auto h-5 w-5 animate-spin text-cyan-200" /><p className="mt-3 text-sm text-slate-400">Loading approved workspace voices…</p></div></div> : voices.data?.length ? <div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3">{voices.data.map(voice => { const isPublicVoice = voice.provider === "huggingface"; return <div className={`rounded-xl border p-4 transition-colors ${selectedVoiceId === voice.id ? "border-violet-300/60 bg-violet-300/8" : "border-white/7 bg-slate-950/35"}`} key={voice.id}><div className="flex items-center gap-2"><div className="grid h-8 w-8 place-items-center rounded-lg bg-violet-400/12"><Mic2 className="h-4 w-4 text-violet-200" /></div><div><p className="text-sm font-medium text-slate-200">{voice.name}</p><p className="text-[10px] uppercase tracking-wide text-slate-500">{voice.language} · {voice.gender}</p></div></div><p className="mt-4 text-xs text-slate-500">{voice.tone || "No tone profile"} · {voice.accent || "Unspecified accent"}</p><span className={`mt-2 inline-block rounded px-1.5 py-1 text-[10px] ${isPublicVoice ? "bg-cyan-300/10 text-cyan-100" : "bg-violet-300/10 text-violet-100"}`}>{isPublicVoice ? "Public model default" : "Private neural quality"}</span><div className="mt-3 flex items-center justify-between gap-2"><span className={`inline-block rounded px-1.5 py-1 text-[10px] ${voice.commercialUse === "allowed" ? "bg-emerald-300/10 text-emerald-200" : "bg-amber-200/10 text-amber-100"}`}>{voice.commercialUse === "allowed" ? "Commercial use allowed" : "License review required"}</span>{voice.providerId ? <Button type="button" variant="outline" size="sm" aria-pressed={selectedVoiceId === voice.id} onClick={() => { setSelectedVoiceId(voice.id); setSpeechResult(null); }} className="h-7 border-violet-200/20 px-2 text-[11px] text-violet-100 hover:bg-violet-200/10">{selectedVoiceId === voice.id ? "Selected" : "Choose voice"}</Button> : <span className="inline-flex items-center gap-1 text-[10px] text-amber-100"><AlertCircle className="h-3 w-3" />Provider unavailable</span>}</div></div>; })}</div> : <div className="grid min-h-52 place-items-center text-center"><div><Mic2 className="mx-auto h-5 w-5 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No voices have been approved for this workspace.</p><p className="mt-1 text-xs text-slate-600">Add provider-backed voices through the administrative registry before use.</p></div></div>}
            </CardContent>
          </Card>
