...
export default function TimelineEditor() {
  ...
  const [productionQuality, setProductionQuality] = useState<"standard" | "high_fidelity">("high_fidelity");
  ...
  useEffect(() => {
    ...
    const selectedProjectId = value ? Number(value) : NaN;
    if (Number.isInteger(selectedProjectId) && selectedProjectId > 0) setProjectId(selectedProjectId);
  }, []);
  ...
  const scenes = draftScenes.length ? draftScenes : (document?.scenes?.length ? document.scenes : scriptScenes);
  ...
  const requestRender = async () => {
    ...
    try {
      const result = await render.mutateAsync({ projectId, versionId: editor.data.id, preset, quality: productionQuality });
  ...
  const saveEditor = async () => {
    if (!projectId) return;
    try {
      await save.mutateAsync({ projectId, scenes, tracks, settings: { aspectRatio: "16:9", exportPreset: preset, productionQuality } });
      setDirty(false);
    ...
    <div className="mx-auto max-w-[1600px] space-y-5 p-5 md:p-8">
      ...
      {!projectId ? <EmptyEditor /> : <>
        <section className="grid gap-5 xl:grid-cols-[1fr_340px]">
          ...
          <Card className="border-cyan-300/10 bg-cyan-300/[.035] shadow-none"><CardContent className="p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-2"><Film className="h-4 w-4 text-cyan-200" /><h2 className="font-medium text-white">Video export</h2></div><p className="mt-1 max-w-2xl text-xs text-slate-500">Choose a browser-local quick draft for immediate review or queue a production MP4 once a private render worker is configured.</p></div><div className="flex flex-wrap items-center gap-2"><select aria-label="Video resolution" value={preset} onChange={(event) => setPreset(event.target.value as typeof preset)} className="h-9 rounded-lg border border-white/10 bg-slate-950/60 px-2 text-xs text-white"><option value="youtube_1080p">Landscape · 1080p</option><option value="vertical_1080x1920">Vertical · 1080 × 1920</option><option value="square_1080">Square · 1080</option></select><select aria-label="Production render quality" value={productionQuality} onChange={(event) => setProductionQuality(event.target.value as typeof productionQuality)} className="h-9 rounded-lg border border-violet-200/20 bg-slate-950/60 px-2 text-xs text-violet-100"><option value="high_fidelity">High fidelity · 30fps · 12 Mbps</option><option value="standard">Standard · 24fps · 8 Mbps</option></select><QuickDraftExportButton title={project.data?.project.title ?? "AI Content OS draft"} scenes={scenes} preset={preset} /><Button type="button" onClick={requestRender} disabled={render.isPending || !editor.data?.id} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">{render.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Film className="mr-2 h-4 w-4" />}Queue production MP4</Button></div></div><div className="mt-4 grid gap-3 text-xs md:grid-cols-2"><div className="rounded-xl border border-cyan-200/10 bg-slate-950/35 p-3"><p className="font-medium text-cyan-100">Browser-local quick draft</p><p className="mt-1 leading-5 text-slate-500">Selectable Preview, Standard, or High WebM. Runs on this device with scene cards and text, supports preview and download, and does not use a natural voice, photoreal footage, or a provider key.</p></div><div className="rounded-xl border border-violet-200/10 bg-slate-950/35 p-3"><p className="font-medium text-violet-100">Production worker MP4 · {productionQuality === "high_fidelity" ? "high fidelity" : "standard"}</p><p className="mt-1 leading-5 text-slate-500">High fidelity requests 30fps H.264 at 12 Mbps with 48 kHz / 192 kbps AAC. It requires a private CPU/GPU worker, approved neural TTS, licensed media models, FFmpeg composition, and object storage. This is the path for natural voice, realistic footage, and long-form jobs approaching one hour.</p></div></div>{exportsQuery.data?.length ? <div className="mt-4 space-y-2">{exportsQuery.data.map((item) => <div className="flex items-center justify-between gap-3 rounded-lg border border-white/7 bg-slate-950/35 p-3" key={item.id}><div><p className="text-xs font-medium text-slate-200">{item.title}</p><p className="mt-1 text-[10px] text-slate-500">Completed production export · {new Date(item.createdAt).toLocaleString()}</p></div><Button type="button" size="sm" variant="outline" onClick={() => setDownloadAssetId(item.id)} className="border-cyan-200/20 bg-transparent text-cyan-100 hover:bg-cyan-300/10"><Download className="mr-1.5 h-3.5 w-3.5" />Download</Button></div>)}</div> : <p className="mt-4 text-xs text-slate-600">Quick drafts download directly to this device. Completed private-worker MP4 exports appear here when configured.</p>}</CardContent></Card>
