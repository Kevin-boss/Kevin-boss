...
import { CalendarDays, CalendarClock, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, FilePenLine, LayoutList, Loader2, RotateCcw, Send, ShieldCheck, Sparkles, Unplug, XCircle } from "lucide-react";
...
export default function Publishing() {
  ...
  const retryDispatch = trpc.production.social.retryDispatch.useMutation();
  ...
  const retryPlan = async (postId: number) => {
    try {
      await retryDispatch.mutateAsync({ postId });
      await posts.refetch();
      toast.success("Retry queued locally. External publication remains inactive until the platform is activated.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not queue a retry for this plan.");
    }
  };
...
function Agenda({ posts, accounts, assigning, onAssignAccount, cancelling, onCancel, rescheduling, onReschedule, retrying, onRetry }: { posts: Array<{ id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }>; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void; cancelling: boolean; onCancel: (postId: number, planTitle: string) => void; rescheduling: boolean; onReschedule: (postId: number, scheduledFor: string) => void; retrying: boolean; onRetry: (postId: number) => void }) {
  return posts.length ? <div className="divide-y divide-white/6">{posts.map(post => <PlanRow post={post} accounts={accounts} assigning={assigning} onAssignAccount={onAssignAccount} cancelling={cancelling} onCancel={onCancel} rescheduling={rescheduling} onReschedule={onReschedule} retrying={retrying} onRetry={onRetry} key={post.id} />)}</div> : <div className="grid min-h-72 place-items-center text-center"><div><CalendarDays className="mx-auto h-6 w-6 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No scheduled plans in the current agenda.</p><p className="mt-1 text-xs text-slate-600">Create an approval-ready plan with a date to add it here.</p></div></div>;
}

function PlanRow({ post, accounts, assigning, onAssignAccount, cancelling, onCancel, rescheduling, onReschedule, retrying, onRetry }: { post: { id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void; cancelling: boolean; onCancel: (postId: number, planTitle: string) => void; rescheduling: boolean; onReschedule: (postId: number, scheduledFor: string) => void; retrying: boolean; onRetry: (postId: number) => void }) {
  ...
  return (
    <div className="flex gap-4 p-5">
      <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${platformColors[post.platform]}`}><Send className="h-4 w-4" /></div>
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-center justify-between gap-2"><p className="truncate text-sm font-medium text-slate-200">{planTitle}</p><span className={`rounded-full px-2.5 py-1 text-[10px] capitalize ${tone}`}>{post.status.replace(/_/g, " ")}</span></div>
        <p className="mt-1 text-xs text-slate-500">{platformLabels[post.platform]}{post.scheduledFor ? ` · ${new Date(post.scheduledFor).toLocaleString()}` : " · unscheduled draft"}</p>
        {post.copy && <p className="mt-3 line-clamp-2 text-xs leading-5 text-slate-400">{post.copy}</p>}
        {canRetry && <Button type="button" size="sm" variant="outline" disabled={retrying} onClick={() => onRetry(post.id)} className="mt-3 border-amber-200/20 text-amber-100 hover:bg-amber-300/10">{retrying ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="mr-1.5 h-3.5 w-3.5" />}Queue retry</Button>}
        {matchingAccounts.length > 0 && <label className="mt-3 block text-[11px] text-slate-500"><span className="mb-1 block">Connected {platformLabels[post.platform]} account</span><select aria-label={`Connected ${platformLabels[post.platform]} account for ${planTitle}`} value={post.socialAccountId ?? ""} disabled={assigning} onChange={event => { const accountId = Number(event.target.value); if (accountId) onAssignAccount(post.id, accountId); }} className="h-8 w-full rounded-md border border-white/10 bg-slate-950/60 px-2 text-xs text-slate-200 outline-none disabled:opacity-60"><option value="">Select connected account</option>{matchingAccounts.map(account => <option value={account.id} key={account.id}>{account.accountName}</option>)}</select></label>}
        {canCancel && <>
          <label className="mt-3 block text-[11px] text-slate-500"><span className="mb-1 block">Local schedule time</span><div className="flex gap-2"><Input aria-label={`Local schedule time for ${planTitle}`} type="datetime-local" value={rescheduleFor} onChange={event => setRescheduleFor(event.target.value)} disabled={rescheduling} className="h-8 border-white/10 bg-slate-950/60 text-xs text-white" /><Button type="button" size="sm" variant="outline" disabled={rescheduling || !rescheduleFor} onClick={() => onReschedule(post.id, rescheduleFor)} className="h-8 shrink-0 border-cyan-200/20 text-cyan-100 hover:bg-cyan-300/10">{rescheduling ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Save time"}</Button></div></label>
          <AlertDialog><AlertDialogTrigger asChild><Button type="button" size="sm" variant="outline" disabled={cancelling} className="mt-3 border-rose-200/20 text-rose-100 hover:bg-rose-300/10"><XCircle className="mr-1.5 h-3.5 w-3.5" />Cancel plan</Button></AlertDialogTrigger><AlertDialogContent className="border-white/10 bg-slate-950 text-slate-100"><AlertDialogHeader><AlertDialogTitle>Cancel {planTitle}?</AlertDialogTitle><AlertDialogDescription>This stops any local queued attempt and removes its saved dispatch schedule. Published and in-flight plans cannot be cancelled here.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={cancelling}>Keep plan</AlertDialogCancel><AlertDialogAction disabled={cancelling} onClick={() => onCancel(post.id, planTitle)} className="bg-rose-300 text-slate-950 hover:bg-rose-200">{cancelling ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <XCircle className="mr-2 h-4 w-4" />}Cancel plan</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
        </>}
      </div>
    </div>
  );
