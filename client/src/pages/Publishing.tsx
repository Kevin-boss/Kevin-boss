    ...
    <div className="min-h-full bg-[#0b1020]">
      ...
      <div className="mx-auto max-w-[1600px] space-y-6 p-5 md:p-8">
        <section className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
          ...
          <Card className="border-white/7 bg-white/[.025] shadow-none">
            <CardContent className="p-0">
              ...
              {mode === "calendar" ? <CalendarGrid days={calendarDays} posts={scheduledPosts} /> : <Agenda posts={scheduledPosts} accounts={accounts.data ?? []} assigning={assignAccount.isPending} onAssignAccount={assignAccountToPost} />}
            </CardContent>
          </Card>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="border-white/7 bg-white/[.025] shadow-none"><CardContent className="p-0"><div className="flex items-center justify-between border-b border-white/7 p-5"><div><h2 className="font-medium text-white">Review queue</h2><p className="mt-1 text-xs text-slate-500">Plans remain transparent about approval and connection prerequisites.</p></div><ShieldCheck className="h-5 w-5 text-amber-100" /></div><div className="divide-y divide-white/6">{posts.data?.length ? posts.data.slice(0, 6).map(post => <PlanRow post={post} accounts={accounts.data ?? []} assigning={assignAccount.isPending} onAssignAccount={assignAccountToPost} key={post.id} />) : <div className="grid min-h-40 place-items-center text-center"><div><ShieldCheck className="mx-auto h-5 w-5 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No plans are awaiting review.</p></div></div>}</div></CardContent></Card>
        </section>
      </div>
    </div>
  );
...
function Agenda({ posts, accounts, assigning, onAssignAccount }: { posts: Array<{ id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }>; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void }) {
  return posts.length ? <div className="divide-y divide-white/6">{posts.map(post => <PlanRow post={post} accounts={accounts} assigning={assigning} onAssignAccount={onAssignAccount} key={post.id} />)}</div> : <div className="grid min-h-72 place-items-center text-center"><div><CalendarDays className="mx-auto h-6 w-6 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No scheduled plans in the current agenda.</p><p className="mt-1 text-xs text-slate-600">Create an approval-ready plan with a date to add it here.</p></div></div>;
}

function PlanRow({ post, accounts, assigning, onAssignAccount }: { post: { id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void }) {
  ...
  return <div className="flex gap-4 p-5"><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${platformColors[post.platform]}`}><Send className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><p className="truncate text-sm font-medium text-slate-200">{post.title || "Untitled platform post"}</p><span className={`rounded-full px-2.5 py-1 text-[10px] capitalize ${tone}`}>{post.status.replace(/_/g, " ")}</span></div><p className="mt-1 text-xs text-slate-500">{platformLabels[post.platform]}{post.scheduledFor ? ` · ${new Date(post.scheduledFor).toLocaleString()}` : " · unscheduled draft"}</p>{post.copy && <p className="mt-3 line-clamp-2 text-xs leading-5 text-slate-400">{post.copy}</p>}{matchingAccounts.length > 0 && <label className="mt-3 block text-[11px] text-slate-500"><span className="mb-1 block">Connected {platformLabels[post.platform]} account</span><select aria-label={`Connected ${platformLabels[post.platform]} account for ${post.title || "untitled plan"}`} value={post.socialAccountId ?? ""} disabled={assigning} onChange={event => { const accountId = Number(event.target.value); if (accountId) onAssignAccount(post.id, accountId); }} className="h-8 w-full rounded-md border border-white/10 bg-slate-950/60 px-2 text-xs text-slate-200 outline-none disabled:opacity-60"><option value="">Select connected account</option>{matchingAccounts.map(account => <option value={account.id} key={account.id}>{account.accountName}</option>)}</select></label>}</div></div>;
}
