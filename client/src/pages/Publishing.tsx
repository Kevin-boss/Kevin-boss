        ...
        <section className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
          ...
          <Card className="border-white/7 bg-white/[.025] shadow-none">
            <CardContent className="p-0">
              ...
              {mode === "calendar" ? <CalendarGrid days={calendarDays} posts={scheduledPosts} /> : <Agenda posts={scheduledPosts} accounts={accounts.data ?? []} assigning={assignAccount.isPending} onAssignAccount={assignAccountToPost} cancelling={cancelDispatch.isPending} onCancel={cancelPlan} />}
            </CardContent>
          </Card>
        </section>
...
function Agenda({ posts, accounts, assigning, onAssignAccount, cancelling, onCancel }: { posts: Array<{ id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }>; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void; cancelling: boolean; onCancel: (postId: number, planTitle: string) => void }) {
  return posts.length ? <div className="divide-y divide-white/6">{posts.map(post => <PlanRow post={post} accounts={accounts} assigning={assigning} onAssignAccount={onAssignAccount} cancelling={cancelling} onCancel={onCancel} key={post.id} />)}</div> : <div className="grid min-h-72 place-items-center text-center"><div><CalendarDays className="mx-auto h-6 w-6 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No scheduled plans in the current agenda.</p><p className="mt-1 text-xs text-slate-600">Create an approval-ready plan with a date to add it here.</p></div></div>;
}

function PlanRow({ post, accounts, assigning, onAssignAccount, cancelling, onCancel }: { post: { id: number; platform: Platform; title: string | null; copy: string | null; status: string; scheduledFor: Date | string | null; socialAccountId: number | null }; accounts: Array<{ id: number; platform: Platform; accountName: string; connectionStatus: string }>; assigning: boolean; onAssignAccount: (postId: number, accountId: number) => void; cancelling: boolean; onCancel: (postId: number, planTitle: string) => void }) {
