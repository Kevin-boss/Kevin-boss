...
import { getFallbackChain, getTaskContract } from "../taskPolicy";
...
export const providerRouter = router({
  ...
  preferred: protectedProcedure.input(z.object({ capability: capabilitySchema })).query(async ({ ctx, input }) => { requirePlatformAdmin(ctx.user.role); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database is unavailable." }); const rows = await db.select().from(modelProviders).where(eq(modelProviders.enabled, "yes")); return { contract: getTaskContract(input.capability), fallback: getFallbackChain(input.capability), providers: rankFreeFirst(rows, input.capability) }; }),
